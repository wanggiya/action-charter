...
const templateParameterLabel = (name: string) => name === "path" ? "Input file" : name === "target_path" ? "Output file" : name.replaceAll("_", " ");
const templateParameterPlaceholder = (template: RecipeTemplate | undefined, name: string) => {
  const root = template?.parameter_roots[name];
  if (!root) return name;
  return `Filename or relative path (default: ${root}/)`;
};
...
export default function App() {
  ...
  const [dataResources, setDataResources] = useState<DataResourceInventory | null>(null);
  const [selectedPlannerInputs, setSelectedPlannerInputs] = useState<string[]>([]);
  ...
  const [snakemakePlan, setSnakemakePlan] = useState<SnakemakeExportPlan | null>(null);
  const [snakemakeResult, setSnakemakeResult] = useState<SnakemakeExportResult | null>(null);
  const [snakemakeConfirmed, setSnakemakeConfirmed] = useState(false);
  const [snakemakePending, setSnakemakePending] = useState(false);
  const [snakemakeNotice, setSnakemakeNotice] = useState("");
  ...
  const [criticInventory, setCriticInventory] = useState<CriticEvidenceInventory | null>(null);
  const [criticNotice, setCriticNotice] = useState("");
  const [criticTraceReviews, setCriticTraceReviews] = useState<string[]>([]);
  const [selectedCriticTraceEvidence, setSelectedCriticTraceEvidence] = useState("");
  const [criticTraceDrawerCollapsed, setCriticTraceDrawerCollapsed] = useState(false);
  const [criticTraceSaving, setCriticTraceSaving] = useState("");
  const [storedCriticTrace, setStoredCriticTrace] = useState<StoredRecipeTrace | null>(null);
  const [selectedStoredCriticTrace, setSelectedStoredCriticTrace] = useState("");
  const [criticInvocationConfirmed, setCriticInvocationConfirmed] = useState(false);
  const [criticInvocationPending, setCriticInvocationPending] = useState(false);
  const [criticAssessment, setCriticAssessment] = useState<CriticAssessmentResult | null>(null);
  const [criticRecordConfirmed, setCriticRecordConfirmed] = useState(false);
  const [criticRecordPending, setCriticRecordPending] = useState(false);
  const [recordedCriticResult, setRecordedCriticResult] = useState<RecordedCriticResult | null>(null);
  const [releaseId, setReleaseId] = useState("");
  const [releaseAssessment, setReleaseAssessment] = useState<RecipeReleaseAssessment | null>(null);
  const [releaseResult, setReleaseResult] = useState<RecipeReleaseResult | null>(null);
  const [releasePending, setReleasePending] = useState(false);
  const [releaseConfirmed, setReleaseConfirmed] = useState(false);
  const [releaseNotice, setReleaseNotice] = useState("");
  const [criticInvocationNotice, setCriticInvocationNotice] = useState("");
  ...
  const selectedCriticTraceCandidate = useMemo(
    () => criticInventory?.recipe_candidates.find((candidate) => candidate.evidence_name === selectedCriticTraceEvidence) ?? null,
    [criticInventory, selectedCriticTraceEvidence],
  );
  const selectedStoredCriticEvidence = useMemo(
    () => criticInventory?.items.find((item) => item.trace_name === selectedStoredCriticTrace && item.available && item.evidence) ?? null,
    [criticInventory, selectedStoredCriticTrace],
  );
  ...
  useEffect(() => {
    if ((!templatePanelOpen && !plannerOpen) || dataResources) return;
  ...
  const openSavedRecipes = async (preferredRecipeSha = "") => {
    ...
    setSnakemakePlan(null);
    setSnakemakeResult(null);
    setSnakemakeConfirmed(false);
    setSnakemakeNotice("");
    setSnakemakeNotice("");
  ...
  const openCriticEvidence = async () => {
    setTemplatePanelOpen(false);
    setRecipePanelOpen(false);
    setExecutionInventoryOpen(false);
    setPlannerOpen(false);
    setCriticOpen(true);
    setCriticNotice("Building deterministic assurance summaries…");
    try {
      const inventory = await loadCriticEvidence();
      setCriticInventory(inventory);
      setSelectedCriticTraceEvidence((current) => current || inventory.recipe_candidates.find((candidate) => candidate.adaptable && !candidate.stored)?.evidence_name || "");
      setSelectedStoredCriticTrace((current) => current || inventory.items.find((item) => item.available)?.trace_name || "");
      setCriticNotice(inventory.item_count ? "" : "No WorkflowTrace and matching report pairs are available yet.");
    } catch {
      setCriticInventory(null);
      setCriticNotice("Critic evidence could not be loaded. Confirm that the local interface service is running.");
    }
  };
  const storeCriticTrace = async (evidenceName: string, traceSha256: string) => {
    setCriticTraceSaving(evidenceName);
    setCriticNotice("Rebuilding and verifying the exact reviewed trace…");
    try {
      const stored = await saveAdaptedRecipeTrace(evidenceName, traceSha256);
      setStoredCriticTrace(stored);
      setCriticNotice("Adapted trace and deterministic report stored immutably. Critic not invoked; no release created.");
      setCriticInventory(await loadCriticEvidence());
    } catch (error) {
      setCriticNotice(error instanceof Error ? error.message : "Adapted trace could not be stored.");
    } finally {
      setCriticTraceSaving("");
    }
  };
  const invokeCritic = async () => {
    if (!selectedStoredCriticEvidence?.evidence || !selectedStoredCriticEvidence.report_name) return;
    const traceReference = selectedStoredCriticEvidence.evidence.evidence_references.find((item) => item.path.endsWith(`/${selectedStoredCriticEvidence.trace_name}`));
    const reportReference = selectedStoredCriticEvidence.evidence.evidence_references.find((item) => item.path.endsWith(`/${selectedStoredCriticEvidence.report_name}`));
    if (!traceReference || !reportReference) {
      setCriticInvocationNotice("Exact stored evidence digests are unavailable.");
      return;
    }
    setCriticInvocationPending(true);
    setCriticInvocationNotice("Calling the read-only Critic and validating its assessment…");
    setCriticAssessment(null);
    try {
      const result = await runCriticAssessment({ traceName: selectedStoredCriticEvidence.trace_name, reportName: selectedStoredCriticEvidence.report_name, traceSha256: traceReference.sha256, reportSha256: reportReference.sha256 });
      setCriticAssessment(result);
      setCriticRecordConfirmed(false);
      setRecordedCriticResult(null);
      setCriticInvocationCollapsed(true);
      setCriticTraceDrawerCollapsed(true);
      setCriticInvocationNotice("Critic assessment validated in memory. Nothing recorded and no release created.");
    } catch (error) {
      setCriticInvocationNotice(error instanceof Error ? error.message : "Critic assessment failed.");
    } finally {
      setCriticInvocationPending(false);
    }
  };
  const recordCriticResult = async () => {
    if (!criticAssessment || !selectedStoredCriticEvidence?.evidence || !selectedStoredCriticEvidence.report_name) return;
    const traceReference = selectedStoredCriticEvidence.evidence.evidence_references.find((item) => item.path.endsWith(`/${selectedStoredCriticEvidence.trace_name}`));
    const reportReference = selectedStoredCriticEvidence.evidence.evidence_references.find((item) => item.path.endsWith(`/${selectedStoredCriticEvidence.report_name}`));
    if (!traceReference || !reportReference) {
      setCriticInvocationNotice("Exact stored evidence digests are unavailable.");
    ...
    setCriticRecordPending(true);
    setCriticInvocationNotice("Revalidating evidence and recording the exact reviewed Critic result…");
    try {
      const recorded = await recordCriticAssessment({ traceName: selectedStoredCriticEvidence.trace_name, reportName: selectedStoredCriticEvidence.report_name, traceSha256: traceReference.sha256, reportSha256: reportReference.sha256, assessment: criticAssessment });
      setRecordedCriticResult(recorded);
      setReleaseId(`release-${recorded.task_id}`.slice(0, 128));
      setReleaseAssessment(null);
      setReleaseResult(null);
      setReleaseConfirmed(false);
      setCriticInvocationNotice("Exact Critic result recorded immutably. No release created and nothing executed.");
    } catch (error) {
      setCriticInvocationNotice(error instanceof Error ? error.message : "Critic result could not be recorded.");
    } finally {
      setCriticRecordPending(false);
    }
  };
  const prepareRelease = async () => {
    if (!recordedCriticResult || !selectedStoredCriticEvidence?.report_name) return;
    setReleasePending(true); setReleaseNotice("Reverifying the complete evidence chain and preparing operational history…");
    try {
      const assessed = await prepareRecipeRelease({ releaseId, traceName: selectedStoredCriticEvidence.trace_name, reportName: selectedStoredCriticEvidence.report_name, criticRecordFile: recordedCriticResult.record_file, criticRecordSha256: recordedCriticResult.critic_record_sha256 });
      setReleaseAssessment(assessed); setReleaseResult(null); setReleaseConfirmed(false);
      setReleaseNotice(assessed.candidate.ready_for_release ? "Exact candidate is release-ready. Review its digest before creating the immutable package." : "Release is blocked by deterministic findings.");
    } catch (error) { setReleaseNotice(error instanceof Error ? error.message : "Release readiness assessment failed."); }
    finally { setReleasePending(false); }
  };
  const createRelease = async () => {
    if (!recordedCriticResult || !selectedStoredCriticEvidence?.report_name || !releaseAssessment) return;
    setReleasePending(true); setReleaseNotice("Reassessing exact evidence and creating the immutable release…");
    try {
      const released = await createRecipeRelease({ releaseId, traceName: selectedStoredCriticEvidence.trace_name, reportName: selectedStoredCriticEvidence.report_name, criticRecordFile: recordedCriticResult.record_file, criticRecordSha256: recordedCriticResult.critic_record_sha256, candidateSha256: releaseAssessment.candidate_sha256, assessedAt: releaseAssessment.candidate.assessed_at });
      setReleaseResult(released); setReleaseNotice("Immutable release created from the exact reviewed candidate.");
    } catch (error) { setReleaseNotice(error instanceof Error ? error.message : "Immutable release creation failed."); }
    finally { setReleasePending(false); }
  };
  ...
  const createPlan = async () => {
    ...
    if (!submittedRequest) {
      setPlannerNotice("Enter a task request before generating a plan.");
      return;
    }
    if (!selectedPlannerSkills.length) return;
    setPlannerRequest(submittedRequest);
    setPlannerPending(true);
    setPlannerRetryAvailable(false);
    ...
    try {
      ...
      if (error instanceof PlannerRequestError) {
        ...
        setPlannerRetryAvailable(error.detail.retryable);
      } else {
  ...
  const resumeSavedPlan = async (item: SavedPlanInventory["plans"][number]) => {
    ...
    requestAnimationFrame(() => {
      if (plannerRequestRef.current) plannerRequestRef.current.value = item.planner_result.original_request;
    });
  ...
  const previewApprovedSnakemakeExport = async () => {
    if (!preparedApproval || !recordedApproval || !approvalVerification?.approved || snakemakePending) return;
    setSnakemakePending(true); setSnakemakeNotice("Rebuilding approved authority and previewing the replay package…");
    try {
      const result = await previewSnakemakeExport(preparedApproval, recordedApproval);
      setSnakemakePlan(result); setSnakemakeResult(null); setSnakemakeConfirmed(false);
      setSnakemakeNotice("Export plan validated. Nothing exported or executed.");
    } catch (error) { setSnakemakeNotice(error instanceof Error ? error.message : "Snakemake export preview failed."); }
    finally { setSnakemakePending(false); }
  };
  const createApprovedSnakemakeExport = async () => {
    if (!preparedApproval || !recordedApproval || !snakemakePlan || !snakemakeConfirmed || snakemakePending) return;
    setSnakemakePending(true); setSnakemakeNotice("Revalidating approval, exporting, and checking the static contract…");
    try {
      const result = await exportSnakemakeRecipe(preparedApproval, recordedApproval, snakemakePlan);
      setSnakemakeResult(result); setSnakemakeNotice(result.export_already_existed ? "The exact export already existed and passed static validation again. Nothing executed." : "Replay package exported and statically validated. Nothing executed.");
    } catch (error) { setSnakemakeNotice(error instanceof Error ? error.message : "Snakemake export failed."); }
    finally { setSnakemakePending(false); }
  };
  ...
  return <main className="app-shell">
      ...
      <section className="template-workspace" role="dialog" aria-modal="true" aria-labelledby="template-title">
        ...
        {recipeTemplates.length ? <div className="template-layout">
          ...
          <div className="template-form">
            ...
            {selectedTemplate?.required_parameters.map((name) => <label key={name}>{templateParameterLabel(name)}{name === "path" && dataResources?.inputs.length ? <select value={dataResources.inputs.some((item) => item.path === templateParameters[name]) ? templateParameters[name] : ""} onChange={(event) => { setTemplateParameters((current) => ({ ...current, path: event.target.value })); setCompilation(null); setReviewConfirmed(false); setSavedRecipe(null); }}><option value="">Select existing input…</option>{dataResources.inputs.map((item) => <option key={item.path} value={item.path}>{item.name} · {item.path}</option>)}</select> : null}{name === "target_path" && dataResources ? <><div className="output-location-fields"><select aria-label="Output folder" value={dataResources.output_directories.find((directory) => templateParameters[name]?.startsWith(`${directory}/`)) ?? "data/output"} onChange={(event) => { const filename = (templateParameters[name] ?? "").replaceAll("\\", "/").split("/").pop() ?? ""; setTemplateParameters((current) => ({ ...current, target_path: filename ? `${event.target.value}/${filename}` : "" })); }}><option disabled>Output folder</option>{dataResources.output_directories.map((directory) => <option key={directory} value={directory}>{directory}</option>)}</select><input aria-label="Output filename" value={(templateParameters[name] ?? "").replaceAll("\\", "/").split("/").pop() ?? ""} maxLength={255} placeholder="result.gpkg" onChange={(event) => { const current = templateParameters[name] ?? ""; const directory = dataResources.output_directories.find((candidate) => current.startsWith(`${candidate}/`)) ?? "data/output"; setTemplateParameters((values) => ({ ...values, target_path: event.target.value ? `${directory}/${event.target.value}` : "" })); setCompilation(null); setReviewConfirmed(false); setSavedRecipe(null); }}/></div><details className="advanced-path"><summary>Use an explicit relative output path</summary><input value={templateParameters[name] ?? ""} maxLength={2000} placeholder="data/output/nested/result.gpkg" onChange={(event) => { setTemplateParameters((current) => ({ ...current, target_path: event.target.value })); setCompilation(null); setReviewConfirmed(false); setSavedRecipe(null); }}/></details></> : <input value={templateParameters[name] ?? ""} maxLength={2000} placeholder={templateParameterPlaceholder(selectedTemplate, name)} onChange={(event) => { setTemplateParameters((current) => ({ ...current, [name]: event.target.value })); setCompilation(null); setReviewConfirmed(false); setSavedRecipe(null); }}/>} {selectedTemplate.parameter_roots[name] && <small className="path-root-hint">Choose a project resource above, or enter a safe relative path manually.</small>}</label>)}
    ...
    {criticOpen && criticAssessment && <aside className="critic-record-drawer" aria-label="Record reviewed Critic result"><header><span><p className="eyebrow">Immutable evidence boundary</p><h3>{releaseResult ? "Authoritative release" : recordedCriticResult ? "Release readiness" : "Record exact assessment"}</h3></span><ShieldCheck size={18}/></header>{recordedCriticResult ? <><section className="critic-recorded"><strong>{releaseResult ? "RELEASED" : "CRITIC RESULT RECORDED"}</strong><span>{releaseResult?.release_directory ?? recordedCriticResult.record_file}</span><code title={releaseResult?.release_sha256 ?? recordedCriticResult.critic_record_sha256}>{releaseResult?.release_sha256 ?? recordedCriticResult.critic_record_sha256}</code><small>{releaseResult ? "Exact evidence copied into an immutable release · nothing executed" : "Immutable result evidence created · no release yet"}</small></section>{!releaseResult && <section className="release-readiness"><label>Release ID<input value={releaseId} disabled={releasePending || Boolean(releaseAssessment)} onChange={(event) => setReleaseId(event.target.value)}/></label>{!releaseAssessment ? <button disabled={releasePending || !releaseId} onClick={() => void prepareRelease()}>{releasePending ? "Verifying evidence…" : "Assess release readiness"}</button> : <><div className={`release-gate ${releaseAssessment.candidate.ready_for_release ? "ready" : "blocked"}`}><strong>{releaseAssessment.candidate.ready_for_release ? "READY · NOT RELEASED" : "RELEASE BLOCKED"}</strong><span>{releaseAssessment.candidate.components.length} exact components · approval {releaseAssessment.candidate.approval_complete ? "complete" : "incomplete"} · validation {releaseAssessment.candidate.validation_complete ? "passed" : "incomplete"} · Critic {releaseAssessment.candidate.critic_complete ? "matched" : "mismatched"}</span><code title={releaseAssessment.candidate_sha256}>{releaseAssessment.candidate_sha256}</code>{releaseAssessment.candidate.violations.length > 0 && <ul>{releaseAssessment.candidate.violations.map((finding) => <li key={finding}>{finding}</li>)}</ul>}</div>{releaseAssessment.candidate.ready_for_release && <><label><input type="checkbox" checked={releaseConfirmed} disabled={releasePending} onChange={(event) => setReleaseConfirmed(event.target.checked)}/><span>I reviewed this exact candidate digest and authorize immutable release creation.</span></label><button disabled={!releaseConfirmed || releasePending} onClick={() => void createRelease()}>{releasePending ? "Reverifying and releasing…" : "Create exact release"}</button></>}</>}{releaseNotice && <p className="critic-invocation-notice">{releaseNotice}</p>}<small>Release packaging only · no tools rerun · no data mutation</small></section>}</> : <><div className="critic-record-scope"><span>Reviewed result SHA-256</span><code title={criticAssessment.critic_result_sha256}>{criticAssessment.critic_result_sha256}</code></div><label><input type="checkbox" checked={criticRecordConfirmed} disabled={criticRecordPending} onChange={(event) => setCriticRecordConfirmed(event.target.checked)}/><span>I reviewed this exact Critic result and authorize immutable evidence recording only.</span></label><button disabled={!criticRecordConfirmed || criticRecordPending} onClick={() => void recordCriticResult()}>{criticRecordPending ? "Revalidating and recording…" : "Record exact Critic result"}</button><small>No model call · no release · no execution</small></>}</aside>}
    {plannerOpen && <div className="template-overlay" role="presentation" onPointerDown={(event) => { if (event.target === event.currentTarget && !plannerPending) setPlannerOpen(false); }}>
      <section className="planner-workspace" role="dialog" aria-modal="true" aria-labelledby="planner-title">
        ...
        <div className="planner-body">
          ...
          <button className="planner-submit" disabled={plannerPending || !selectedPlannerSkills.length} onClick={() => void createPlan()}>{plannerPending ? "Planning and validating…" : plannerRetryAvailable ? "Retry plan after reviewing finding" : "Generate validated plan"}</button>
          {plannerNotice && <p className={`planner-notice ${plannerRetryAvailable ? "planner-rejected" : ""}`}>{plannerNotice}</p>}
    ...
    {recipePanelOpen && recordedApproval && <section className={`authority-outcome-dock decision-${recordedApproval.decision}`} aria-live="polite">
      ...
      {approvalVerification?.approved && <section className="snakemake-export-control"><header><Workflow size={20}/><span><small>Reproducible replay</small><strong>Snakemake export</strong></span><em>NO EXECUTION</em></header>{!snakemakePlan ? <button disabled={snakemakePending} onClick={() => void previewApprovedSnakemakeExport()}>{snakemakePending ? "Revalidating authority…" : "Preview approved export"}</button> : <><dl><div><dt>Recipe</dt><dd>{snakemakePlan.plan.recipe_id}</dd></div><div><dt>Files</dt><dd>Snakefile · replay config · manifest</dd></div></dl><code title={snakemakePlan.export_plan_sha256}>{snakemakePlan.export_plan_sha256}</code>{snakemakeResult ? <div className="snakemake-exported"><CheckCircle2 size={20}/><span><strong>{snakemakeResult.export_already_existed ? "EXISTING EXPORT · CONTRACT VALID" : "EXPORTED · CONTRACT VALID"}</strong><small>{snakemakeResult.export.export_path}</small></span></div> : <><label><input type="checkbox" checked={snakemakeConfirmed} disabled={snakemakePending} onChange={(event) => setSnakemakeConfirmed(event.target.checked)}/><span>I reviewed this exact export plan and authorize replay-package creation only.</span></label><button disabled={!snakemakeConfirmed || snakemakePending} onClick={() => void createApprovedSnakemakeExport()}>{snakemakePending ? "Exporting and validating…" : "Export and validate"}</button></>}</>}{snakemakeNotice && <p className={snakemakeResult?.export_already_existed ? "snakemake-existing-notice" : ""}>{snakemakeNotice}</p>}<small>Static package only · Snakemake and recipe are not run</small></section>}
